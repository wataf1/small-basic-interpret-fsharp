...
type PropertyBuilders = {
    propBuilder:PropertyBuilder;
    fieldBuilder:FieldBuilder;
    getBuilder:MethodBuilder;
    setBuilder:MethodBuilder;
...
let generateProperties (typeBuilder:TypeBuilder) (instructions:Instruction array) =
    ...
    [for instruction in instructions do
        match instruction with
        | Field(name,_) -> yield name
        | StartField(name) -> yield name
        | _ -> ()]
    |> List.distinct
    |> List.map (fun x -> x, generateProperty x)
    |> dict
let getParamTypes (args:'a list) = args |> List.map (fun _ ->typeof<Primitive>) |> Array.ofList
...
type ILGenerator with
    ...
    member il.EmitNewPrimitive(t:Type) =
        if t = typeof<char> then
            let ci = typeof<Primitive>.GetConstructor([|typeof<string>|])
            il.Emit(OpCodes.Newobj,ci)
        else 
            let ci = typeof<Primitive>.GetConstructor([|t|])
            il.Emit(OpCodes.Newobj,ci)

    member il.EmitCtor<'a>() = 
            let ci = typeof<Primitive>.GetConstructor([|typeof<'a>|])
            il.Emit(OpCodes.Newobj,ci)

    //member il.EmitPrimitive<'a when 'a:struct> (v:'a)= //<'a when 'a:struct> (v:'a) =
    //    match box v with
    //    | :? bool as b -> 
    //        if b then il.Emit(OpCodes.Ldc_I4_1)
    ...
    //        il.EmitCtor<int64>()
    //    | :? float as f ->
    //        il.Emit(OpCodes.Ldc_R8,f) // pushes supplied value of type float64 onto stack
    //        il.EmitCtor<float>()
    //    | :? string as s ->
...
let compileTo name program =
    ...
    emitInstructions il doc methods fields properties program
    il.Emit(OpCodes.Ret)
